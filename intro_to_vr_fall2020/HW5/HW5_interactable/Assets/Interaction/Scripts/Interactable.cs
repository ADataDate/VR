...
namespace CUVR{
    public class Interactable : MonoBehaviour
    {
        ...
        public virtual void HandleUpdate(){
            if (!ping && !prevPing) //not being looked at 
            {
                state = State.Waiting;
            }
            else if (ping && !prevPing) //just got looked at 
            {
                state = State.Enter;
            }
            else if (ping && prevPing) //looked at for more than one frame 
            {
                state = State.Hover;
            }
            else if (!ping && prevPing)//starting to look away 
            {
                state = State.Exit;
            }
